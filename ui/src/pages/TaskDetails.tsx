          ...
          <Card>
            ...
            <CardContent className="space-y-2">
                <div className="flex justify-between py-1 border-b">
                    ...
                    <span className="font-medium">{task.flow_version_id}</span>
                </div>
                <div className="flex justify-between py-1 border-b">
                    <span className="text-muted-foreground">Current Node</span>
                    <span className="font-medium">{task.current_node_key || "-"}</span>
                </div>
                <div className="flex justify-between py-1 border-b">
                ...
                <div className="flex justify-between py-1 border-b">
                    ...
                    <span className="font-medium">{new Date(task.created_at * 1000).toLocaleString()}</span>
                </div>
                <div className="flex justify-between py-1 border-b">
                    <span className="text-muted-foreground">Updated At</span>
                    <span className="font-medium">{new Date(task.updated_at * 1000).toLocaleString()}</span>
                </div>
            </CardContent>
          </Card>
              ...
              <TableBody>
                {runs.map((run) => (
                  <TableRow key={run.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground">{run.worker_id}</TableCell>
                    <TableCell>{new Date(run.started_at * 1000).toLocaleTimeString()}</TableCell>
                    <TableCell>
                        {run.finished_at > 0 ? `${(run.finished_at - run.started_at).toFixed(3)}s` : "-"}
                    </TableCell>
