    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Version: {version.version} | ID: {version.id}</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
           <pre className="bg-muted p-4 rounded text-xs font-mono overflow-auto max-h-[300px]">
              {JSON.stringify(JSON.parse(version.definition_json || "{}"), null, 2)}
           </pre>
        </CardContent>
      </Card>
              ...
              <TableBody>
                {tasks.map((task) => (
                  <TableRow key={task.id}>
                    ...
                    <TableCell>
                      <Badge variant={task.status === "completed" ? "default" : task.status === "failed" ? "destructive" : task.status === "running" ? "secondary" : "outline"}>
                        {task.status}
                      </Badge>
                    </TableCell>
                    <TableCell>{task.current_node_key || "-"}</TableCell>
                    <TableCell>{task.step_count}</TableCell>
                    <TableCell>{new Date(task.updated_at * 1000).toLocaleString()}</TableCell>
