...
func (e *Engine) runWaitEvent(t store.Task, def FlowDef, node DefNode, curr string, shared map[string]interface{}, params map[string]interface{}, input interface{}) error {
    ...
    if we == nil { we = map[string]interface{}{"start": time.Now().UnixMilli()} }
    signalKey := ""
    if v, ok := params["signal_key"].(string); ok { signalKey = v }
    sig := resolveRef(signalKey, shared, params, input)
    timeout := 0
    if v, ok := params["timeout_ms"].(float64); ok { timeout = int(v) } else if v2, ok := params["timeout_ms"].(int); ok { timeout = v2 }
    if sig != nil && sig != "" && sig != false {
        action := node.Post.ActionStatic
        if action == "" && node.Post.ActionKey != "" { action = pickAction(map[string]interface{}{"signal": sig}, node.Post.ActionKey) }
        if node.Post.OutputKey != "" { shared[node.Post.OutputKey] = sig }
        delete(rt, key)
    ...
    start := int64(0)
    if s, ok := we["start"].(int64); ok { start = s } else if s2, ok := we["start"].(float64); ok { start = int64(s2) }
